...
(function () {
  ...
  var STATE_CONFIG = {
    ...
    pending:        { icon: "⏳", title: "Almost there",          message: "Your account is pending approval. We'll message you once it's active.", tone: "warn", anim: "anim-pop", actions: [{ label: "Close" }] },
  ...
  // ==========================================================================
  // Kenyan phone: 07XXXXXXXX / 01XXXXXXXX / 2547XXXXXXXX / 2541XXXXXXXX / +254...
  function isValidKenyanPhone(raw) {
    var digits = raw.replace(/[\s\-()]/g, "").replace(/^\+/, "");
    return /^(254(7|1)\d{8}|0(7|1)\d{8})$/.test(digits);
  }
  ...
  function isValidEmail(raw) {
    // Backup check beyond the browser's built-in type="email" validation —
    // catches things like "name@site" (no TLD) or stray spaces some
    // voice-to-text keyboards insert.
    return /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(raw.trim());
  }

  var PASSWORD_RULES = [
    { key: "len",     test: function (p) { return p.length >= 8; } },
    { key: "upper",   test: function (p) { return /[A-Z]/.test(p); } },
    { key: "lower",   test: function (p) { return /[a-z]/.test(p); } },
    { key: "digit",   test: function (p) { return /[0-9]/.test(p); } },
    { key: "special", test: function (p) { return /[^A-Za-z0-9]/.test(p); } }
  ];
  function passwordMeetsAllRules(pass) {
    return PASSWORD_RULES.every(function (r) { return r.test(pass); });
  }
  ...
  function bindLogin() {
    ...
    var modeSigninBtn = document.getElementById("modeSigninBtn");
    var modeSignupBtn = document.getElementById("modeSignupBtn");
    ...
    var loginDemoNote = document.getElementById("loginDemoNote");
    ...
    if (toggleBtn) {
      toggleBtn.addEventListener("click", function () {
        ...
        toggleBtn.querySelector(".eye-open").hidden = !showing ? false : true;
        toggleBtn.querySelector(".eye-closed").hidden = !showing ? true : false;
        toggleBtn.setAttribute("aria-pressed", String(!showing));
        toggleBtn.setAttribute("aria-label", showing ? "Show password" : "Hide password");
      });
    }

    // ---- live password strength checklist (signup mode only) ----
    passInput.addEventListener("input", function () {
      if (mode !== "signup" || !passwordRulesList) return;
      var val = passInput.value;
      PASSWORD_RULES.forEach(function (r) {
        var li = passwordRulesList.querySelector('[data-rule="' + r.key + '"]');
        if (li) li.classList.toggle("is-met", r.test(val));
      });
    });
    ...
    function setMode(next) {
      ...
      modeSigninBtn.classList.toggle("is-active", !isSignup);
      modeSigninBtn.setAttribute("aria-selected", String(!isSignup));
      modeSignupBtn.classList.toggle("is-active", isSignup);
      modeSignupBtn.setAttribute("aria-selected", String(isSignup));
      signupFields.forEach(function (f) { f.hidden = !isSignup; });
      if (passwordRulesList) passwordRulesList.hidden = !isSignup;
      rememberRow.style.display = isSignup ? "none" : "flex";
      loginTitle.textContent = isSignup ? "Create your account." : "Welcome back.";
      loginSub.textContent = isSignup
        ? "Set up your login so you can track your project and pay from your phone."
        : "Sign in to see your project, your plan, and your payments — all in one place.";
      loginDemoNote.textContent = isSignup
        ? "🔒 Your password is stored securely by Firebase — we never see it."
        : "🔒 Sign in with the email and password set up for your account.";
      submitLabel.textContent = isSignup ? "Create account" : "Sign in";
      form.reset();
      if (passwordRulesList) {
        passwordRulesList.querySelectorAll("li").forEach(function (li) { li.classList.remove("is-met"); });
      }
    }

    if (modeSigninBtn && modeSignupBtn) {
      modeSigninBtn.addEventListener("click", function () { setMode("signin"); });
      modeSignupBtn.addEventListener("click", function () { setMode("signup"); });
    }

    // ---- after a successful sign-in/sign-up, gate on approval before
    // letting the client into the dashboard ----
    function afterAuth(user, isSignup) {
      return window.VectuzAuth.getClientProfile(user.uid).then(function (profile) {
        if (!profile || profile.approved !== true) {
          window.VectuzAuth.logOut();
          resetSubmitUi(isSignup);
          showState("pending", {
            message: isSignup
              ? "Account created. We review every new client before activating their portal — we'll message you once it's ready."
              : "Your account is still pending approval. Message us on WhatsApp if this is taking a while."
          });
          return;
        }
        enterDashboard(user, profile);
      });
    }

    function resetSubmitUi(isSignup) {
    ...
    if (googleBtn) {
      googleBtn.addEventListener("click", function () {
        if (!window.VectuzAuth) return;
        googleBtn.disabled = true;
        // Navigates the whole page to Google's sign-in screen and back —
        // does NOT resolve here. handleGoogleRedirect() just below runs on
        // every page load and catches the result once Google sends the
        // person back.
        window.VectuzAuth.googleSignIn();
      });
    }

    // Runs on every load of this page. Almost always a no-op (resolves to
    // null) — only does something right after someone comes back from
    // tapping "Continue with Google".
    if (window.VectuzAuth) {
      window.VectuzAuth.handleGoogleRedirect()
        .then(function (user) {
          if (user) return afterAuth(user, true);
        })
        .catch(function (err) {
          showState("error", { message: window.VectuzAuth.friendlyError(err.code) });
        });
    }
    ...
    form.addEventListener("submit", function (e) {
      ...
      var isSignup = mode === "signup";
      var nameVal = isSignup ? document.getElementById("signupName").value.trim() : "";
      var phoneVal = isSignup ? document.getElementById("signupPhone").value.trim() : "";

      var problem = null;
      if (!idVal || !passVal || (isSignup && !nameVal)) {
        problem = isSignup && !nameVal
          ? "Enter your name so we know who's paying."
          : (!idVal ? "Enter your email address." : "Enter a password.");
      } else if (!isValidEmail(idVal)) {
        problem = "That email address doesn't look valid — double check it.";
      } else if (isSignup && phoneVal && !isValidKenyanPhone(phoneVal)) {
        problem = "Enter a valid Kenyan number, e.g. 07XX XXX XXX.";
      } else if (isSignup && !passwordMeetsAllRules(passVal)) {
        problem = "Your password doesn't meet all the requirements below yet.";
      }
      ...
      if (problem) {
        ...
        showState("validation", { message: problem });
        return;
      }

      if (!window.VectuzAuth) {
        showState("error", { message: "Login isn't wired up yet — check firebase-config.js has your real project keys." });
        return;
      }
      ...
      var authPromise = isSignup
        ? window.VectuzAuth.signUp(idVal, passVal, nameVal, normalizeKenyanPhone(phoneVal || ""), "Starter")
        : window.VectuzAuth.logIn(idVal, passVal);

      authPromise
        .then(function (user) { return afterAuth(user, isSignup); })
        .catch(function (err) {
          resetSubmitUi(isSignup);
          card.classList.remove("shake");
          void card.offsetWidth;
    ...
    function enterDashboard(user, profile) {
      ...
      window.setTimeout(function () {
        ...
        window.scrollTo(0, 0);
        setGreeting(user, profile);
        applyProfileToDashboard(profile);
        bindRevealFresh();
      }, 500);
    }

    function setGreeting(user, profile) {
      var raw = (profile && profile.name) || user.displayName || (user.email ? user.email.split("@")[0] : "there");
      raw = raw.replace(/[^a-zA-Z0-9 ]/g, " ").trim();
      var display = raw ? raw.charAt(0).toUpperCase() + raw.slice(1) : "there";
      var greetEl = document.getElementById("portalGreeting");
      if (greetEl) greetEl.textContent = "Welcome back, " + display + ".";
    }

    // Fills the plan/status cards from the client's real Firestore record
    // instead of the hard-coded "Starter / In progress" placeholders.
    function applyProfileToDashboard(profile) {
      if (!profile) return;
      var planEl = document.getElementById("statusPlan");
      if (planEl && profile.plan) planEl.textContent = profile.plan;
      window.__vectuzClient = profile; // used by payments.js
    }
  }
  ...
  function bindLogout() {
    ...
    btn.addEventListener("click", function () {
      ...
      if (window.VectuzAuth) {
        window.VectuzAuth.logOut().then(finish).catch(finish);
      } else {
        finish();
      }
    });

...
face.querySelectorAll('.cubie').forEach(function(cu
bie, ci) {
...
setTimeout(function() {
cubie.style.transform = 'scale(1)'; }, 120);
}, fi * 180 + ci * 55);
});
});
...
cube.style.animation = 'miniCubeSpin 1s
linear infinite';
});
...
{ label: 'Restaurant Site', icon:
'\ud83c\udf7d\ufe0f' },
{ label: 'E-Commerce Store', icon:
'\ud83d\udecd\ufe0f' },
{ label: 'Salon Booking', icon: '\ud83d\udc87' },
{ label: 'Real Estate', icon: '\ud83c\udfe1' },
{ label: 'Fashion Brand', icon: '\ud83d\udc57' },
...
Gallery, Contact and more. Everything your business
needs.' },
...
animations that make your site feel premium and
engaging.' },
{ title: 'WhatsApp Chat Button', desc: 'A
floating WhatsApp button so customers can message
you instantly from any page — increases conversions
significantly.' },
...
API — STK Push, C2B, B2C, and transaction callbacks
all handled.' },
...
{
return '<div class="modal-feature"><div
class="modal-feature-title" style="color:' +
pkg.color + '">✓ ' + f.title + '</div><div
class="modal-feature-desc">' + f.desc + '</div>
</div>';
}).join('');

modalBody.innerHTML = '<div class="modal-
header"><div class="modal-plan-name" style="color:'

+ pkg.color + '">' + pkg.name + '</div><div
class="modal-plan-price">' + pkg.price + '</div><p
class="modal-tagline">' + pkg.tagline + '</p></div>
<div class="modal-features-grid">' + featuresHTML +
'</div><a href="#contact" class="modal-cta"

style="background:' + pkg.color + '" id="modal-cta-
btn">Get Started →</a>';
...
loopActive = true;
// MAX package: purple energy wave effect
var ewaves =
...
setTimeout(function() {
scatterCubeModal(modalCube); }, 300);
setTimeout(function() {
buildCube(modalCube, false);
loopSolve();
}, 1400);
...
face.style.transition = 'transform 0.8s ease,
opacity 0.8s ease';
...
document.querySelectorAll('.service-card, .pricing-
card, .process-step, .why-point, .stat');
...
el.style.transition = 'opacity 0.5s ' + (i *
0.05) + 's ease, transform 0.5s ' + (i * 0.05) + 's
ease';
observer.observe(el);
});
}

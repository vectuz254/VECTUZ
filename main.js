...
function initPackageModals() {
  ...
  function openModal(packageKey) {
    ...
    modalBody.innerHTML =
      ...
      '<a href="#contact" class="modal-cta" style="background:' + pkg.color + '" id="modal-cta-btn">Get Started →</a>';
  ...
  // ── FIX 1: use only 'click' — no touchend double-fire ──
...
/* ── SCROLL REVEAL ── */
// FIX 2: lower threshold + rootMargin so elements actually trigger
function initScrollReveal() {
  ...
  targets.forEach(function (el, i) {
    ...
    el.style.transition =
      'opacity 0.5s ' + (i * 0.05) + 's ease, transform 0.5s ' + (i * 0.05) + 's ease';
    observer.observe(el);
  });
}
...
/* ── THEME TOGGLE ── */
// FIX 3: rewritten so dark↔light actually switches reliably
function initThemeToggle() {
  ...
  // Sync icon with whatever the theme guard already set
